...
const RentalContractDetailPage = () => {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setExpense((current) => ({ ...current, contractId, periodKey: todayKey().slice(0, 7) }));
    } catch (loadError) {
            ...
            {obligations.map((obligation) => {
              ...
              return (
                <article className="border rounded-3 p-3" key={obligation.id}>
                  ...
                  {canManage && !obligation.externalClosure?.closed && <div className="row g-2 align-items-end border-top mt-3 pt-3"><div className="col-sm-6 col-lg-3"><label className="form-label small" htmlFor={`charge-${obligation.id}`}>Otros cargos del período</label><input id={`charge-${obligation.id}`} className="form-control form-control-sm" inputMode="decimal" value={chargeDrafts[obligation.id] ?? minorToMajorInput(obligation.otherChargesMinor)} onChange={(event) => setChargeDrafts({ ...chargeDrafts, [obligation.id]: event.target.value })} /></div><div className="col-sm-6 col-lg-2"><button type="button" className="btn btn-sm btn-outline-secondary" disabled={working} onClick={() => runAction(() => updateRentalObligationCharges({ inmobiliariaId: activeInmobiliariaId, obligationId: obligation.id, otherChargesMinor: majorToMinor(chargeDrafts[obligation.id] ?? minorToMajorInput(obligation.otherChargesMinor)) }), "Cargos actualizados.")}>Actualizar cargos</button></div></div>}
